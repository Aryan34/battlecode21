...
//			}
	...
	public void checkECSafety() throws GameActionException {
		if(earlyScoutingLoc == null){
			return;
		}
		Log.log("Early scouting loc: " + earlyScoutingLoc.toString());
		Direction threatDir = creatorLoc.directionTo(earlyScoutingLoc);
		Direction myDir = creatorLoc.directionTo(myLoc);
		Log.log("Threat dir: " + threatDir.toString());
		Log.log("My dir: " + myDir.toString());
		if(myDir == threatDir || myDir == threatDir.rotateLeft() || myDir == threatDir.rotateRight()){
			// Run towards the other side of the EC
			Direction oppDir = myDir.opposite();
			nav.goTo(creatorLoc.add(oppDir).add(oppDir).add(oppDir));
			Log.log("Going in opp dir: " + oppDir.toString());
		}
	}
