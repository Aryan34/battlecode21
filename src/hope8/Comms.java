...
public class Comms {
    ...
    static int[] parseFlag(int flag){
        ...
        switch(purpose){
            ...
            case 6:
                int[] splits6 = {4, 6, 7, 7};
                return splitFlag(flag, splits6);
            case 7:
                int[] splits7 = {4, 1, 7, 7};
                return splitFlag(flag, splits7);
            case 8:
                int[] splits8 = {4, 1};
                return splitFlag(flag, splits8);
            default:
...
//                Log.log("Reading troop type");
                ...
                break;
            case 6:
                int scoutingDir = splits[1];
                int x2 = splits[2];
                int y2 = splits[3];
                MapLocation testLoc = xyToMapLocation(x2, y2);
                if(rc.getLocation().equals(testLoc)){
                    robot.muckNum = scoutingDir;
                }
            case 7:
                int found = splits[1];
                int x3 = splits[2];
                int y3 = splits[3];
                if(found == 0){
                    robot.earlyScoutingLoc = null;
                }
                else{
                    MapLocation earlyLoc = xyToMapLocation(x3, y3);
                    robot.earlyScoutingLoc = earlyLoc;
                }
                break;
            case 8:
                if(splits[1] == 1){
