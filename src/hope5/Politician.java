...
//		killLargerEnemies(nearby);
	...
	public void killLargerEnemies(RobotInfo[] nearby) throws GameActionException {
		...
		for (int j = 0; j < idx; j++) { // Go through all targets, find who should kill it
			...
			if (thisCanKill) { someoneCanKill = true; }
			...
			if (rc.getConviction() <= minConv && rc.canEmpower(dist)) { // we have the lowest conv, should attack
				tryMovingAttack(enemy.getLocation(), dist);
				// rc.empower(dist); // TODO: Add code to move closer before empowering?
				break;
			}
			if (!someoneCanKill && rc.canEmpower(dist)) { // if no one can kill it, just attack anyway
				tryMovingAttack(enemy.getLocation(), dist);
				// rc.empower(dist);
				break;
			}
	...
	public void tryMovingAttack(MapLocation biggestThreat, int attackDist) throws GameActionException {
		boolean moved = false;
